...
echo "==================================================================================="
echo "Adding $KADMIN_PRINCIPAL principal with password \"$KADMIN_PASSWORD\""
...
echo "Adding noPermissions principal with password \"$KADMIN_PASSWORD\""
...
add_principal () {
  CUSTOM_PRINCIPAL=$1
  CUSTOM_KEYTAB=$2
  echo "Adding $CUSTOM_PRINCIPAL principal with password \"$KADMIN_PASSWORD\""
  ...
  if [[ -n $CUSTOM_KEYTAB ]]; then
    ...
    kadmin.local -q "xst -norandkey -k $CUSTOM_KEYTAB_PATH $CUSTOM_PRINCIPAL@$REALM"
    chmod 666 $CUSTOM_KEYTAB_PATH
    echo ""
  fi
}
add_principal user user.keytab
add_principal hdfs hdfs.keytab
add_principal hdfs/hdfs hdfs-service.keytab
add_principal zookeeper/hbase zookeeper-service.keytab
add_principal hbase/hbase hbase-service.keytab
add_principal hive/hive hive-service.keytab
